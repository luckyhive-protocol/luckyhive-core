...
(define-public (execute-change (proposal-id uint))
  (let (
    ...
    (if (is-eq (get parameter-key proposal) PARAM-DRAW-INTERVAL)
      (try! (contract-call? .luckyhive-prize-pool set-draw-interval (get new-value proposal)))
      (if (is-eq (get parameter-key proposal) PARAM-PAUSE)
        (try! (contract-call? .luckyhive-prize-pool set-paused (> (get new-value proposal) u0)))
        (if (or
          ...
          (try! (contract-call? .luckyhive-prize-pool set-deposit-limits
...
(define-public (emergency-pause)
  (begin
    (asserts! (is-guardian) ERR-NOT-AUTHORIZED)
    (try! (contract-call? .luckyhive-prize-pool set-paused true))
